...
//
// This file implements the kernel compilation functions (see the header for
// more information).
//
// =================================================================================================
...
Program CompileFromSource(
    ...
    const size_t
        run_preprocessor,  // 0: platform dependent, 1: always, 2: never
    const bool silent) {
    auto header_string = std::string{""};

    header_string +=
        "#define PRECISION " + ToString(static_cast<int>(precision)) + "\n";

    // Adds the name of the routine as a define
    header_string += "#define ROUTINE_" + routine_name + "\n";

    // Not all OpenCL compilers support the 'inline' keyword. The keyword is
    // only used for devices on which it is known to work with all OpenCL
    // platforms.
    if (device.IsNVIDIA() || device.IsARM()) {
        header_string += "#define USE_INLINE_KEYWORD 1\n";
    }

    // For specific devices, use the non-IEE754 compliant OpenCL mad()
    // instruction. This can improve performance, but might result in a reduced
    // accuracy.
    ...
    // For specific devices add a global synchronisation barrier to the GEMM
    // kernel to optimize performance through better cache behaviour
...
// Prints details of the routine to compile in case of debugging in verbose mode
    ...
    printf("[DEBUG] Compiling routine '%s-%s'\n", routine_name.c_str(),
           ToString(precision).c_str());
    const auto start_time = std::chrono::steady_clock::now();
#endif

    // Runs a pre-processor to unroll loops and perform array-to-register
    // promotion. Most OpenCL compilers do this, but some don't.
    ...
    fprintf(stderr, "CompileProgram 1\n");

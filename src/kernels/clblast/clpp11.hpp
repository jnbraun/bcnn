...
//
// This file implements a bunch of C++11 classes that act as wrappers around
// OpenCL objects and API calls. The main benefits are increased abstraction,
// automatic memory management, and portability. Portability here means that a
// similar header exists for CUDA with the same classes and interfaces. In other
// words, moving from the OpenCL API to the CUDA API becomes a one-line change.
//
// This file is taken from the CLCudaAPI project
// <https://github.com/CNugteren/CLCudaAPI> and therefore contains the following
...
#define CheckError(call) \
    CLCudaAPIError::Check(call, CLCudaAPIError::TrimCallString(#call))

// Error occurred in OpenCL (no-exception version for destructors)
#define CheckErrorDtor(call) \
    CLCudaAPIError::CheckDtor(call, CLCudaAPIError::TrimCallString(#call))
...
class Event {
   public:
    // Constructor based on the regular OpenCL data-type: memory management is
    // handled elsewhere
    ...
    // (Note that there is a bug in Apple's OpenCL implementation of the
    // 'clGetEventProfilingInfo' function:
    //  http://stackoverflow.com/questions/26145603/clgeteventprofilinginfo-bug-in-macosx)
    // However, in our case the reply size is fixed to be cl_ulong, so we are
    // not affected.
    float GetElapsedTime() const {
        WaitForCompletion();
        const auto bytes = sizeof(cl_ulong);
        auto time_start = cl_ulong{0};
        CheckError(clGetEventProfilingInfo(*event_, CL_PROFILING_COMMAND_START,
                                           bytes, &time_start, nullptr));
        auto time_end = cl_ulong{0};
        CheckError(clGetEventProfilingInfo(*event_, CL_PROFILING_COMMAND_END,
                                           bytes, &time_end, nullptr));
...
class Platform {
   public:
    ...
    explicit Platform(const size_t platform_id) {
        ...
        if (platform_id >= num_platforms) {
            throw RuntimeError("Platform: invalid platform ID " +
    ...
    size_t NumDevices() const {
        ...
        CheckError(
            clGetDeviceIDs(platform_, CL_DEVICE_TYPE_ALL, 0, nullptr, &result));
   ...
   private:
    ...
    std::string GetInfoString(const cl_device_info info) const {
        ...
        result.resize(bytes);
        CheckError(
            clGetPlatformInfo(platform_, info, bytes, &result[0], nullptr));
        result.resize(
            strlen(result.c_str()));  // Removes any trailing '\0'-characters
        return result;
    }
};
...
inline std::vector<Platform> GetAllPlatforms() {
    ...
    for (size_t platform_id = 0;
         platform_id < static_cast<size_t>(num_platforms); ++platform_id) {
...
class Device {
   public:
    ...
    size_t VersionNumber() const {
        ...
        // Space separates the end of the OpenCL version number from the
        // beginning of the vendor-specific information.
        size_t next_whitespace = version_string.find(' ');
        size_t version = (size_t)(
    ...
    size_t CoreClock() const {
        return static_cast<size_t>(
            GetInfo<cl_uint>(CL_DEVICE_MAX_CLOCK_FREQUENCY));
    }
    size_t ComputeUnits() const {
        return static_cast<size_t>(
            GetInfo<cl_uint>(CL_DEVICE_MAX_COMPUTE_UNITS));
    }
    ...
    bool IsIntel() const {
        return Vendor() == "INTEL" || Vendor() == "Intel" ||
               Vendor() == "GenuineIntel" || Vendor() == "Intel(R) Corporation";
    }
    bool IsARM() const { return Vendor() == "ARM"; }
    bool IsQualcomm() const { return Vendor() == "QUALCOMM"; }

    // Platform specific extensions
    std::string AMDBoardName()
        const {  // check for 'cl_amd_device_attribute_query' first
#ifndef CL_DEVICE_BOARD_NAME_AMD
#define CL_DEVICE_BOARD_NAME_AMD 0x4038
#endif
        return GetInfoString(CL_DEVICE_BOARD_NAME_AMD);
    }
    std::string NVIDIAComputeCapability()
        const {  // check for 'cl_nv_device_attribute_query' first
...
#define CL_DEVICE_COMPUTE_CAPABILITY_MINOR_NV 0x4001
        ...
        return std::string{"SM"} +
               std::to_string(
                   GetInfo<cl_uint>(CL_DEVICE_COMPUTE_CAPABILITY_MAJOR_NV)) +
               std::string{"."} +
               std::to_string(
                   GetInfo<cl_uint>(CL_DEVICE_COMPUTE_CAPABILITY_MINOR_NV));
    }
   ...
   public:
    ...
    explicit Program(const Context &context, const std::string &source)
        : program_(new cl_program, [](cl_program *p) {
              if (*p) {
                  CheckErrorDtor(clReleaseProgram(*p));
              }
              delete p;
          }) {
        ...
        auto status = CL_SUCCESS;
        *program_ = clCreateProgramWithSource(context(), 1, &source_ptr,
                                              &length, &status);
    ...
    explicit Program(const Device &device, const Context &context,
        ...
        *program_ = clCreateProgramWithBinary(
            context(), 1, &dev, &length,
            reinterpret_cast<const unsigned char **>(&binary_ptr), &status1,
            &status2);
        CLCudaAPIError::Check(status1,
                              "clCreateProgramWithBinary (binary status)");
        CLCudaAPIError::Check(status2, "clCreateProgramWithBinary");
    }

    // Compiles the device program and checks whether or not there are any
    // warnings/errors
    void Build(const Device &device, std::vector<std::string> &options) {
        ...
        const cl_device_id dev = device();
        fprintf(stderr, "clblast - clBuildProgram options %s ...",
                options_string.c_str());
        CheckError(clBuildProgram(*program_, 1, &dev, options_string.c_str(),
                                  nullptr, nullptr));
        fprintf(stderr, "clBuildProgram success\n");
    }

    // Confirms whether a certain status code is an actual compilation error or
    // warning
    ...
    std::string GetBuildInfo(const Device &device) const {
        ...
        CheckError(clGetProgramBuildInfo(*program_, device(), query, 0, nullptr,
                                         &bytes));
        auto result = std::string{};
        result.resize(bytes);
    ...
    // Retrieves a binary or an intermediate representation of the compiled
    // program
    std::string GetIR() const {
        ...
        CheckError(clGetProgramInfo(*program_, CL_PROGRAM_BINARIES,
                                    sizeof(char *), &result_ptr, nullptr));
        return result;
    }
...
class Queue {
   public:
    ...
    Context GetContext() const {
        auto bytes = size_t{0};
        CheckError(clGetCommandQueueInfo(*queue_, CL_QUEUE_CONTEXT, 0, nullptr,
                                         &bytes));
        cl_context result;
        CheckError(clGetCommandQueueInfo(*queue_, CL_QUEUE_CONTEXT, bytes,
                                         &result, nullptr));
        return Context(result);
    }
    Device GetDevice() const {
        auto bytes = size_t{0};
        CheckError(clGetCommandQueueInfo(*queue_, CL_QUEUE_DEVICE, 0, nullptr,
                                         &bytes));
        cl_device_id result;
        CheckError(clGetCommandQueueInfo(*queue_, CL_QUEUE_DEVICE, bytes,
                                         &result, nullptr));
...
class Buffer {
   public:
    ...
    // Regular constructor with memory management. If this class does not own
    // the buffer object, then the memory will not be freed automatically
    // afterwards. If the size is set to 0, this will become a stub containing a
    // nullptr
    explicit Buffer(const Context &context, const BufferAccess access,
        ...
        auto status = CL_SUCCESS;
        *buffer_ = (size > 0)
                       ? clCreateBuffer(context(), flags, size * sizeof(T),
                                        nullptr, &status)
                       : nullptr;
    ...
    void CopyToAsync(const Queue &queue, const size_t size,
                     const Buffer<T> &destination) const {
        CheckError(clEnqueueCopyBuffer(queue(), *buffer_, destination(), 0, 0,
...
class Kernel {
   public:
    ...
    void SetArgument(const size_t index, const T &value) {
        CheckError(clSetKernelArg(*kernel_, static_cast<cl_uint>(index),
                                  sizeof(T), &value));
    }
    template <typename T>
    ...
    // Sets all arguments in one go using parameter packs. Note that this
    // overwrites previously set arguments using 'SetArgument' or
    // 'SetArguments'.
    ...
    std::string GetFunctionName() const {
        auto bytes = size_t{0};
        CheckError(clGetKernelInfo(*kernel_, CL_KERNEL_FUNCTION_NAME, 0,
                                   nullptr, &bytes));
        auto result = std::string{};
        result.resize(bytes);
        CheckError(clGetKernelInfo(*kernel_, CL_KERNEL_FUNCTION_NAME, bytes,
                                   &result[0], nullptr));
        return std::string{
            result.c_str()};  // Removes any trailing '\0'-characters
    }
